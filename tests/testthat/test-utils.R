...
test_that("power and t2 error calculations work", {
  ...
  expect_equal(names(t2_power_result),
               c('Sample Size', 'test', 'lower_power_bound', 'upper_power_bound', 'power', 'Power 95% CI',
...
test_that("data object names are consistent",{
  ...
  expected_col_names <- c("wilcox", "fisher", "chi_sq_false", "chi_sq_true", "lrm", "kruskal",
...
test_that("data formatting works", {
  ...
  tbl_cols <- c('wilcox', 'fisher', 'chi_sq_false', 'chi_sq_true', 'lrm', 'kruskal',
                'coinasymp', 'run', 'y', 'x', 'n_null', 'n_intervene', 'sample_size', 'K')
