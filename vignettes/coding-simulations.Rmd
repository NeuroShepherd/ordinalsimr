...
sim_results <- run_simulations(sample_size = 80,
                sample_prob = c(0.5,0.5),
                prob0 = c(0.1,0.2,0.3,0.4),
                prob1 = c(0.1,0.2,0.3,0.4), # note the matching probabilities between groups
                niter = 20
                )
...
As a check, it can be noted that the number of rows in this table, or the number of distinct simulations to run, is equivalent to the combination of all parameters `length(sample_size)*length(sample_prob)*length(prob0_list)*length(prob1_list)*length(niter)` or `r length(sample_size)*length(sample_prob)*length(prob0_list)*length(prob1_list)*length(niter)` in this case.
